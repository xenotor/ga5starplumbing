# Docs

One topic per file. Keep entries brief; if a thing is visible in the code, link
to the code instead of restating it.

| File | Covers |
| --- | --- |
| [RULES.md](RULES.md) | the business rules the code serves — read first |
| [architecture.md](architecture.md) | request routing, API surface, data model |
| [scheduling.md](scheduling.md) | business hours, overlap policy, timezone rules |
| [booking-module.md](booking-module.md) | reusing the scheduler on other landing pages |
| [attribution.md](attribution.md) | how Facebook ad clicks are tied to bookings |
| [cloudflare-account.md](cloudflare-account.md) | account isolation, resources, credentials |
| [deployment.md](deployment.md) | CI pipeline, required secrets, manual deploy |
| [local-dev.md](local-dev.md) | env files, running and testing locally |
| [cutover.md](cutover.md) | how the domain was moved off GitHub Pages |
